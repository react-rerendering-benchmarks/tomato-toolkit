...
import { Button, Card, Image, Message, Tabs, Typography } from '@arco-design/web-react';
...
const MyBatisPlusGenerator: FC = memo(() => {
  const store = useStore();
  const generateMyBatisPlus = async () => {
    try {
      await invoke('mp_codegen', {
  ...
  return <Card className="h-full p-3" cover={<>
        <div className="flex items-center space-x-3 px-4">
          <Image src={mybatisIcon} width={64} preview={false} />
          <Typography.Title heading={4}>MyBatisPlus代码生成器</Typography.Title>
        </div>
      </>}>
      <DataSourceConfig />

      <Tabs type="rounded" animation lazyload destroyOnHide={false} size="large" defaultActiveTab="GlobalConfig">
      ...
      <Button long type="primary" size="large" className="mt5" disabled={!store.mp.isInclude} onClick={generateMyBatisPlus}>
        生成
      </Button>
    </Card>;
});
