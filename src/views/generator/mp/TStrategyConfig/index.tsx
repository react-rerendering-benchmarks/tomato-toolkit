...
import { Form, Button, Radio, Select, Grid, Space, Tabs, Tag, Message, Modal } from '@arco-design/web-react';
...
const TStrategyConfig: FC = memo(() => {
  ...
  reaction(() => store.mp.strategyStore.isHydrated, isHydrated => {
    if (isHydrated) {
      form.setFieldsValue({
        ...store.mp.strategyStore.strategyBase
      });
    }
  });
  useEffect(() => {
  ...
  return <>
      <Form<StrategyBaseConfig> colon form={form} layout="vertical" initialValues={store.mp.strategyStore.strategyBase} onValuesChange={(_, vs) => {
      ...
      <Form form={tableForm} colon layout="vertical" initialValues={{
      include: store.mp.strategyStore.include,
      exclude: store.mp.strategyStore.exclude
    }} onValuesChange={(_, vs) => {
          ...
          <Grid.Row>
            <Grid.Col span={20}>
              <Form.Item field="include">
                <Select placeholder="请选择" mode="multiple" dragToSort allowClear options={basicTables.map(option => ({
                label: <div className="flex justify-between items-center px-3">
              ...
              }))} renderFormat={option => <div className="flex items-center">
      ...
      <Tabs type="rounded" animation lazyload destroyOnHide={false} size="large" defaultActiveTab="EntityConfig">
      ...
      <Modal title="预览生成的数据" unmountOnExit visible={visiblePreview} hideCancel onOk={() => setVisiblePreview(false)} onCancel={() => setVisiblePreview(false)} style={{
      width: '80%',
      height: '90vh'
    }}>
        <Editor language="json" height="68vh" options={{
        fontSize: 18,
        folding: true
      }} theme="vs-dark" defaultValue={code} loading={<IconLoading spin />} />
      </Modal>
    </>;
});
