...
import { Button, Form, Grid, Input, Message, Modal, Radio, Space, Table, TableColumnProps, Tag, Tooltip } from '@arco-design/web-react';
...
const InjectConfig: FC = memo(() => {
  ...
  const columns: TableColumnProps<CustomFile>[] = [{
    ...
    width: 160,
    ellipsis: true,
    render: (_, item) => item.filePath || <Tooltip content={`${store.mp.outputDir}（默认路径）`}>
         <Tag color="#ff7d00">
    ...
    render: (col, item) => <Tag color={item.fileOverride ? '#00b42a' : '#f28c18'}>
          {item.fileOverride ? '是' : '否'}
        </Tag>
  }, {
    title: 'entity前缀',
    dataIndex: 'addEntityPrefix',
    render: (col, item) => <Tag color={item.addEntityPrefix ? '#00b42a' : '#f28c18'}>
    ...
    render(col, item, _index) {
      return <Space>
          <Button size="mini" type="primary" icon={<IconEdit />} onClick={() => handleEditCustomFile(item)} />
          <Button size="mini" status="danger" icon={<IconDelete />} onClick={() => handleDelete(item)} />
        </Space>;
    }
  }];
  ...
  return <div className="space-y-5">
      ...
      <Table<CustomFile> columns={columns} data={store.mp.injectStore.customFiles} />

      <Modal title="添加自定义数据" visible={visibleCustomMap} unmountOnExit onOk={handleOk} onCancel={() => setVisibleCustomMap(false)} style={{
      height: '90vh',
      width: '80%'
    }}>
        <Editor height="68vh" options={options} theme="vs-dark" defaultLanguage="json" value={customMap} onChange={value => setCustomMap(value || '')} loading={<IconLoading spin />} />
      </Modal>

      <Modal title={modalTitle} visible={visibleCustomFile} unmountOnExit onOk={handleCustomFileOk} onCancel={() => setVisibleCustomFile(false)} style={{
      width: '80%'
    }}>
        <Form<CustomFile> form={form} layout="vertical" autoComplete="off" colon initialValues={customFile} onValuesChange={(_, vs) => {
        setCustomFile({
          ...customFile,
          ...vs
        });
          ...
          <Grid.Row gutter={24}>
            <Grid.Col xs={24} md={12}>
              <Form.Item label="模板路径" required>
                <Grid.Row>
                  <Grid.Col span={20}>
                    <Form.Item field="templatePath" rules={[{
                    required: true,
                    message: '模板路径必填'
                  }]}>
            ...
            <Grid.Col xs={24} md={12}>
              <Form.Item field="fileName" label="文件名" rules={[{
              required: true,
              message: '文件名必填'
            }]}>
