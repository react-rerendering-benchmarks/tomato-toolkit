...
import { Form, Input, Select, Radio, Button, Grid, Message } from '@arco-design/web-react';
...
const TGlobalConfig: FC = memo(() => {
  ...
  reaction(() => store.mp.globalStore.isHydrated, isHydrated => {
    if (isHydrated) {
      form.setFieldsValue({
        ...store.mp.globalStore.global
      });
    }
  });
  return <Form<GlobalConfig> colon form={form} layout="vertical" initialValues={store.mp.globalStore.global} onValuesChange={v => {
    store.mp.globalStore.setGlobalConfig(v);
      ...
      <Form.Item itemRef="outputDir" label="输出目录">
        <Grid.Row>
          ...
          <Grid.Col xs={6} md={4}>
            <Form.Item>
              <Button long type="primary" onClick={selectDirectory}>
                选择目录
              </Button>
            </Form.Item>
          </Grid.Col>
