...
const Breadcrumb: FC = memo(() => {
  ...
  let breadcrumbs = useMatches().map(match => ({
    ...match,
    path: match.pathname
  } as RouteMenu));
  ...
  return <ArcoBreadcrumb className="flex-1 ml-5">
      {breadcrumbs.map(breadcrumb => <BreadcrumbItem key={breadcrumb.handle.name}>
          <div className="flex items-center space-x-2">
            {breadcrumb.handle.icon && breadcrumb.handle.icon}
            {isHome(breadcrumb) || !breadcrumb.handle.submenu && resolvePath({
          pathname: breadcrumb.path
        }).pathname !== location.pathname ? <Link to={{
          pathname: breadcrumb.path || '/'
        }}>
                  {breadcrumb.handle.title}
                </Link> : <span>{breadcrumb.handle.title}</span>}
          </div>
        </BreadcrumbItem>)}
    </ArcoBreadcrumb>;
});
